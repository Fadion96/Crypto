...
def create_matrix_from_knapsack(knap, cipher):
    n = len(knap)
    result = np.identity(n, dtype=int)
    result = np.vstack((result, knap))
    result = np.column_stack((result, [0]*n + [-cipher]))
...
def create_matrix(n):
    row = len(n)
    col = len(n[0])
    res = [ [n[i][j] for j in range(col) ] for i in range(row)]
    return np.array(res) + Fraction()
